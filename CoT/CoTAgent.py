...
class CoTAgent:
    ...
    def structured_base_prompt(self):
        ...
        prompt = prompt.partial(
            tool_names=", ".join([tool.name for tool in self.get_tools()])
        )
    ...
    def instanciate_ai_model(self):
        ...
        tool_executor = self.tool_executor  # noqa

        functions = [convert_to_openai_function(t) for t in self.get_tools()]

        model = prompt | ChatOpenAI(model="gpt-4-0125-preview").bind_functions(
            functions
        )
